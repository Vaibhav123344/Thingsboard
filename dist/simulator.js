...
function generateSensorTelemetry(deviceName) {
    const keys = deviceTelemetryKeys[deviceName] || ['temperature', 'humidity', 'pressure', 'vibration'];
    const telemetry = {};
    for (const key of keys) {
        if (key === 'temperature')
            telemetry[key] = parseFloat((Math.random() * (85.0 - 20.0) + 20.0).toFixed(2));
        else if (key === 'humidity')
            telemetry[key] = parseFloat((Math.random() * (90.0 - 30.0) + 30.0).toFixed(2));
        else if (key === 'pressure')
            telemetry[key] = parseFloat((Math.random() * (1.5 - 0.9) + 0.9).toFixed(3));
        else if (key === 'vibration')
            telemetry[key] = parseFloat((Math.random() * (4.5 - 0.01) + 0.01).toFixed(2));
        else {
...
async function autoProvisionDevices() {
    ...
    try {
        ...
        const devicesRes = await tbClient.request('GET', '/api/tenant/devices', { pageSize: 100, page: 0 });
...
async function startSimulatorAPI() {
    ...
    app.get('/simulation/alarms', async (req, res) => {
        try {
